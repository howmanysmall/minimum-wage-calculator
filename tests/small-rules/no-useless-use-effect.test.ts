...
import rule from "@oxlint-rules/no-useless-use-effect";
...
`,
			...
			// Async named function declaration used as effect callback (valid)
			{
				code: `
import { useEffect, useState } from "@rbxts/react";
...
function Component() {
    ...
    useEffect(initEffect, [x]);
}
`,
			},

			// Inline async function expression as effect callback (valid)
			{
				code: `
import { useEffect } from "@rbxts/react";
