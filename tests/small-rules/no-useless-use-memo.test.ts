...
import rule from "@oxlint-rules/no-useless-use-memo";
...
describe("no-useless-use-memo", () => {
	...
	ts.run("no-useless-use-memo", rule, {
		invalid: [
			// Single-statement block: single ReturnExpression (invalid)
			{
				code: `
import { useMemo } from "react";

const value = useMemo(() => { return 1; }, []);
`,
...
`,
			...
			// Single-statement block: non-return statement (valid)
			{
				code: `
import { useMemo } from "react";

const value = useMemo(() => { const x = 1; }, []);
`,
				options: [{ environment: "standard" }],
			},
			// Single-statement block: return with no argument (valid)
			{
				code: `
import { useMemo } from "react";
