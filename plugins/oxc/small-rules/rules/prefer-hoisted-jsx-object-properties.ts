...
import { isModuleLevelScope, isStaticExpression } from "@oxlint-utilities/static-expression-utilities";
import { defineRule } from "oxlint-plugin-utilities";

import type { StaticExpressionOptions } from "@oxlint-utilities/static-expression-utilities";
import type { Context, ESTree, Scope, Visitor } from "oxlint-plugin-utilities";
...
function isJsxElementAssignedToModuleConst(
	context: { sourceCode: { getScope(node: ESTree.Node): null | Scope } },
	...
	while (true) {
		...
		const parent: WalkableParent["parent"] = current.parent;
...
const preferHoistedJsxObjectProperties = defineRule({
	...
	meta: {
		docs: {
			description:
				"Prefer extracting inline React JSX object props to module-level constants when the entire object is statically hoistable.",
		},
		messages: {
			hoistableObjectProp:
				"Extract `{{objectText}}` to a module-level const — this inline React JSX prop object is fully static and causes unnecessary reference churn on every render.",
		},
		schema: [],
		type: "suggestion",
	},
