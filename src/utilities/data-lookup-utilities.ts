...
import { isRentSnapshot } from "@project-types";
import { type } from "arktype";

import type { FoodPlanTier, FoodSnapshot, HouseholdProfile, VersionSnapshot, ZipRentRecord } from "@project-types";
...
export function normalizeZip(value: string): string {
	return value.replaceAll(/\D/gu, "").slice(0, 5);
}
...
async function loadZipRentMapAsync(): Promise<Map<string, ZipRentRecord>> {
	...
	if (!response.ok) {
		const error = new TypeError(`Failed to load rent snapshot: HTTP ${response.status}`);
		Error.captureStackTrace(error, loadZipRentMapAsync);
		throw error;
	}

	const rentSnapshot = isRentSnapshot(await response.json());
	if (rentSnapshot instanceof type.errors) {
		const error = new TypeError(`Rent snapshot payload is invalid: ${rentSnapshot.summary}`);
		Error.captureStackTrace(error, loadZipRentMapAsync);
		throw error;
	}
