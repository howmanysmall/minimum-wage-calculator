...
await Bun.write(HUD_XLSX_PATH, result);

console.log("Building ZIP rent snapshot JSON...");
await buildHudSnapshotAsync(Bun.file(HUD_XLSX_PATH), RENT_SNAPSHOT_PATH, HUD_YEAR, HUD_URL);
...
await Bun.write(VERSION_PATH, `${JSON.stringify(versionPayload, undefined, 2)}\n`, { createPath: true });
